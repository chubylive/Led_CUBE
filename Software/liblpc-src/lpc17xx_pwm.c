...
void PWM_ConfigStructInit(uint8_t PWMTimerCounterMode, void *PWM_InitStruct)
{
	...
	if (PWMTimerCounterMode == PWM_MODE_TIMER )
	{
		...
		//remember to possibly change if faster speeds needed
...
void PWM_Init(LPC_PWM_TypeDef *PWMx, uint32_t PWMTimerCounterMode, void *PWM_ConfigStruct)
{
	...
	//I will be using PWM1 SO!
	set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_0, PINSEL_FUNC_1); //enable PWM1 on P2.0
	...
	// the clock divide is set to 4 
	// so we should be operating at a clock speed of 25mhz
