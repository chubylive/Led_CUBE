		...
		if (((LPC_UART1_TypeDef *) UARTx) == LPC_UART1 ) {
			...
			((LPC_UART1_TypeDef *) UARTx)->FDR =
					(UART_FDR_MULVAL(mulFracDivOptimal)
							| UART_FDR_DIVADDVAL(dividerAddOptimal) )
							& UART_FDR_BITMASK;
		} else {
...
void UART_Init(LPC_UART_TypeDef *UARTx, UART_CFG_Type *UART_ConfigStruct) {
	...
	if (((LPC_UART1_TypeDef *) UARTx) == LPC_UART1 ) {
		...
		set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_0, PINSEL_FUNC_2); //enable TxD1 on P2.0
		set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_1, PINSEL_FUNC_2); //enable RxD1 on P2.1
		set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_2, PINSEL_FUNC_2); //enable CTS on P2.2
		set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_7, PINSEL_FUNC_2); //enable RTS on P2.7
	...
	if (((LPC_UART1_TypeDef *) UARTx) == LPC_UART1 ) {
		...
		/* FIFOs are empty */
		UARTx->/*IIFCR.*/FCR = (UART_FCR_FIFO_EN | UART_FCR_RX_RS
				| UART_FCR_TX_RS );
		// Disable FIFO
		UARTx->/*IIFCR.*/FCR = 0;
...
void UART_DeInit(LPC_UART_TypeDef* UARTx) {
	...
	if (((LPC_UART1_TypeDef *) UARTx) == LPC_UART1 ) {
		...
		set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_0, PINSEL_FUNC_0); //enable TxD1 on P2.0
		set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_1, PINSEL_FUNC_0); //enable RxD1 on P2.1
		set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_2, PINSEL_FUNC_0); //enable CTS on P2.2
		set_PinFunc(PINSEL_PORT_2, PINSEL_PIN_7, PINSEL_FUNC_0); //enable RTS on P2.7
 ...
 * @return		None
 *******************************************************************************/
void UART_ConfigStructInit(UART_CFG_Type *UART_InitStruct, uint32_t Baudrate) {
 ...
 **********************************************************************/
uint32_t UART_Send(LPC_UART_TypeDef *UARTx, uint8_t *txbuf, uint16_t buflen, TRANSFER_BLOCK_Type flag) {
...
void UART_IntConfig(LPC_UART_TypeDef *UARTx, UART_INT_Type UARTIntCfg,
	...
	if ((LPC_UART1_TypeDef *) UARTx == LPC_UART1 ) {
		CHECK_PARAM(
				(PARAM_UART_INTCFG(UARTIntCfg)) || (PARAM_UART1_INTCFG(UARTIntCfg)));
	} else {
...
void UART_RS485Config(LPC_UART1_TypeDef *UARTx,
	...
	CHECK_PARAM(
			PARAM_UART1_RS485_CFG_DELAYVALUE(RS485ConfigStruct->DelayValue));
	...
	if (RS485ConfigStruct->AutoAddrDetect_State == ENABLE) {
		...
		// Fill Match Address
		UARTx->ADRMATCH = RS485ConfigStruct->MatchAddrValue
				& UART1_RS485ADRMATCH_BITMASK;
	}
